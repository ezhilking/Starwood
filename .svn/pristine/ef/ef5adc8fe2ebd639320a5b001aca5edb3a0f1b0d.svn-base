...
public class REG18_FreeNightAward {
	CRM SW = new CRM();	
	...
	@Test(priority=1)
	public void CreateFreeNightReservation() {
		try
		{
		String RequestXMLFile= "WC_REG18_FreeNightsRes.xml";
		...
		String Cancelpolicydate=SW.DateAddDays(SW.GetTimeStamp("yyyy-MM-dd"), "yyyy-MM-dd", 10, Calendar.DATE);//one day prior to arrival date
	...
	@Test(priority=2, dependsOnMethods="CreateFreeNightReservation")
	public void dbValidtion()
	{
		try{	
			...
			CheckQuery="select * from freq_travel_call_comm where call_comm_Cd='WCT' and mbrshp_num in ("+spgMem+")and call_comm_text =''";
			if(SW.RecordExists(CheckQuery))
