 ...
 * TestCase Name: PostStaySPGMemberGLevel
...
public class SPGLink_REG26_PostStaySPGMemberGLevel {
	CRM SW = new CRM();
	String Mbrshp_num;
	String FirstName;
	String LastName;
	...
	@Test(priority=0)
	public void DataSetupPostStayGold() {
		SW.EstablishConnection("QA3");
		String Query="select m.mbrshp_num , p.first_name , p.last_name from odsft.freq_travel_mbrshp_prof p, odsft.freq_travel_mbrshp m where m.mrktg_pgm = 5 and m.mbrshp_lvl = 'G' and m.mbrshp_sts = 'A' and m.join_date > (sysdate-30) and m.gst_master_prof_id != 0 and m.mbrshp_num=p.mbrshp_num";
	...
	public void PostStaySPGMemberGLevel(){
		...
		SW.DropDown_SelectByText("SPGLinkPostSPGStay_WelcomeGift_EB", "Internet");
