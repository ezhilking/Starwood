...
/** Purpose		: This is to Update SPG member in SPG Link 2.0 under SPG Pro User Role
 * TestCase Name: UpdateMemberProfile
 * Created By	: Veeresh
 * Modified By	: 
 * Modified Date: 
 * Reviewed By	: 
...
public class SPGLink_REG02_UpdateMemberProfile {
	...
	public void UpdateMember() {
		...
		SW.EnterValue("SPGLinkLookUp_EnterMbrNumber_EB",
				SW.TestData("SPGNumber"));
		SW.Click("SPGLinkLookUp_ClickSearch_BT");

		// Edit Member Profile
		SW.Click("SPGLinkLookUp_ClickEdit_BT");
		SW.ClearValue("SPGLinkUpdate_EditEmail_EB");
		SW.EnterValue("SPGLinkUpdate_EditEmail_EB", 
				SW.TestData("SPGLinkEmailid"));
		SW.Click("SPGLinkUpdate_Submit_BT");

		if (SW.ObjectExists("SPGLinkAdmin_RoleChangeAlert_DT")) {

			String UpdateNumber = SW.GetText("SPGLinkAdmin_RoleChangeAlert_DT");
			UpdateNumber = UpdateNumber.substring(UpdateNumber.lastIndexOf(" "),
					UpdateNumber.indexOf("."));
			Environment.loger.log(Level.INFO, "Member Updated Successfully!!"
					);
		}else {
			Environment.loger.log(Level.ERROR, "Member Update fails");
			SW.FailCurrentTest("Member Update failed");
		}
	}
