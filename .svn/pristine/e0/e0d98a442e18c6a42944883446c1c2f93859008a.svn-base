 ...
 * TestCase Name:  Reg_Corp User_Create or Modify RP_Verify the error message displayed when rate plan differential type and Sings are matched with the Governor selected and differential value entered is lower than the Governor minimum..
...
public class MUSTRUN37_CreateorModifyRP_VerifyErrorMessageDisplayedWhenDifferentialTypeDiffersFromGovernor {
	CRS SW = new CRS();
	...
	@Test(priority=0)
		...
		SW.DropDown_SelectByText("VPRates_ChildRPDiffType_DD", "Fixed");
		SW.EnterValue("VPRates_ChildRPSinglediff_EB", "15");
		SW.VPClick("VPRates_DynamicSeasonSave_BT");
		SW.CompareTextContained("VPRates_GovernorDifftypeErrorValidation", "Differential indicator between Governor & Rate Plan must match (Percent(%) vs Fixed).", SW.GetText("VPRates_ErrMsg_DT").trim());
		SW.DropDown_SelectByText("VPRates_ChildRPDiffType_DD", "Percent");
		SW.NormalClick("VPRates_ChildRPDiffOffsetSign_BT");
		SW.VPClick("VPRates_DynamicSeasonSave_BT");
		SW.CompareTextContained("VPRates_GovernorDifftypeErrorValidation", "Differential signs between Governor & Rate Plan must match (Negative(-) vs Positive(+)).", SW.GetText("VPRates_ErrMsg_DT").trim());
		SW.NormalClick("VPRates_ChildRPDiffOffsetSign_BT");
		SW.VPClick("VPRates_DynamicSeasonSave_BT");
