...
/** Purpose		: Validate that Property Description is displayed while creating AHBB D2 offer Westin In English language when GC setup has it
 * TestCase Name: Validate that Property Description is displayed while creating AHBB D2 offer Westin In English language when GC setup has it
 * Created By	: Sachin
 * Modified By	: 
 * Modified Date: 
 * Reviewed By	:	
...
public class GC_REG26_ValidateEditingActiveGCSetupAddingSocialMediaIconsManagerTitleInReservationEmail {

	CRM SW = new CRM();	
	String ReservationNumber;
	String TestCaseName= getClass().getName();
	String Username,Password;
	String RequestXMLFile,PropertyID;
	...
	public void StartTest(){
		...
		if(Environment.getRunEnvironment().equalsIgnoreCase("QA3")){
			RequestXMLFile= "GC_REG26_Booking Request_QA3_1967.xml";
			PropertyID="1967";
		}
	...
	public void ValidateEditGCSetup(){
		...
		SW.Click("GCSetUp_ChangeOfferImage_LK");
		String InitialSelectedImage=SW.GetText("GCSetUp_SelectedImage_DT");
		Environment.loger.log(Level.INFO, "First time selected offer Image :"+InitialSelectedImage );
		//Select a Image which is not selected already
		SW.NormalClick("(//li[@id='dividimage1select.property.imageHead']/ul//a[not(text()='"+InitialSelectedImage+"')])[1]");
		String EditedSelectedImage=SW.GetText("GCSetUp_SelectedImage_DT");
		Environment.loger.log(Level.INFO, "Second time selected offer Image :"+EditedSelectedImage );

		SW.WriteToEmailTestData(TestCaseName, "ValiadtionString1", EditedSelectedImage);
		...
		String SelectedManagerTitle=SW.DropDown_GetSelectedText("GCSetUp_ManagerTitle_DD");

		Environment.loger.log(Level.INFO, "Selected Manager Title :"+SelectedManagerTitle );

		SW.WriteToEmailTestData(TestCaseName, "ValiadtionString2", SelectedManagerTitle);
