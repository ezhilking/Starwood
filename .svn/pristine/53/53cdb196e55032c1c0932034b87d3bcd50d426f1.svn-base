...
public class MUSTRUN14_VPPropertyContentCategoryType {
	
	CRS SW =new CRS();
	String SucessfullyCreated;
	String CategoryType= SW.RandomString(5).toUpperCase();
	...
	public void PropertyContent(){
		...
		SW.EnterValue("VP_MenuSearch_EB","Category Type");
		SW.Click("VPPropContent_CatogeryTypeSelected_BT");
		SW.SwitchToFrame("VP_MainFrame_FR");
		...
		public void CreateQS(){
			...
			SW.EnterValue("VPPropContent_AttractionType_ST", CategoryType);
		...
		public  void Db(){
			try{	
				...
				String query="select catg_name from prop.catg where catg_name='"+ CategoryType +"'";
				Reporter.WriteLog(Level.INFO, query);
				if(SW.RecordExists(query)){
					List<String> output = SW.GetColumnValues(query, 1);{
						if (SW.CompareText(output.get(0), CategoryType)) {
