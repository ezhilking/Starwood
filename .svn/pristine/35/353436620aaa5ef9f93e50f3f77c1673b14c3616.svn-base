...
public class CHANNELS extends Utility{
	...
	public String GetEventName(String ActualEventName){
		NavigateTo(Environment.WEBOUTLOOK);
		String MailID = TestData("email");
		EnterValue("Outlook_EmailInput_EB", MailID);
		Click("Outlook_Password_EB");
		...
		Click("Outlook_SignIn_BT");
		Click("Outlook_SearchBox_BT");
		EnterValue("Outlook_SearchBox_ET", ActualEventName);
		Click("Outlook_Email_Searched_BT");
		String MailPassword  =  GetText("Outlook_ClientName_ST");
		String [] Expected =   MailPassword.split(":");
		// Reporter.WriteLog(Level.INFO, "eventname", Expected[1].trim());
		String ExpectedEventname =  Expected[1].trim();
