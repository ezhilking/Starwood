...
/** Purpose		:	Notification Triggered when SPG WeChat Member reach Platinum and P50
 * TestCase Name:	REG09_ReachPlatinumandP50.java
...
public class REG08_ReachPlatinumandP50 {
	...
	public void dbValidtion()
	{
		try{	
			...
			String CheckQuery="select * from odsft.freq_travel_notf_hist where mbrshp_num in ("+spgMem+") and message_type_id in ('891','894','895')";
			...
			CheckQuery="select * from freq_travel_call_comm where call_comm_Cd='WCT' and mbrshp_num in ("+spgMem+")and call_comm_text in ('ACCOUNT UPGRADED OR RENEWED.','MEMBER EARNED P50 BENEFITS','STARPOINTS POSTED FOR A STAY.')";
			if(SW.RecordExists(CheckQuery))
