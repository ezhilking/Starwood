...
public class MUSTRUN26_VPPropertyContentCompassDirection {
	
	CRS SW =new CRS();
	String SucessfullyCreated;
	String CompassName= SW.RandomString(5).toUpperCase();
	...
	public void PropertyContent(){
		...
		SW.EnterValue("VP_MenuSearch_EB","Compass Direction");
		SW.Click("VPPropContent_CompassDirection_BT");
		SW.SwitchToFrame("VP_MainFrame_FR");
	...
	//	SW.DropDown_SelectByValue("VPPropContent_CatogeryTypeOwnerId_DD", "2");
		SW.EnterValue("VPPropContent_CompassID_ET", CompassID);
		SW.EnterValue("VPPropContent_CompassName_ET",CompassName );
	...
	public  void Db(){
		try{	
			...
			String query="select compass_point_name from odsref.compass_point_lkup where compass_point_name='"+ CompassName +"'";
			Reporter.WriteLog(Level.INFO, query);
			if(SW.RecordExists(query)){
				List<String> output = SW.GetColumnValues(query, 1);{
					if (SW.CompareText(output.get(0), CompassName)) {
