...
public class MUSTRUN23_CorpSup_Saratoga_SearchMsg_Create_Approve_ConditionalScript {
	CRS SW = new CRS();
	...
	SW.Click("VPCorp_ScriptUserRole_RB");
	...
	SW.Click("VPCorp_ScriptSelectApprove_BN");
	SW.CompareText("Pre Approval Message Validation", "Status changed to Locked-Pending Approval", SW.GetText("VPRates_RPinfoMsg_DT").trim());
	SW.Click("VPCorp_ScriptfinalApprove_BN");
	SW.CompareText("Final Approval Messgae Validation", "Conditional Script is Successfully Approved", SW.GetText("VPRates_RPinfoMsg_DT").trim());
	
	
	// Deleting approved Message
	SW.Click("VPCorp_ScriptCancel_BN");
	SW.DropDown_SelectByText("VPCorp_ScriptType_DD", "Conditional Script");
	SW.EnterValue("VPCorp_ScriptMsgText_EB", "Automated Conditional Type Script "+CategoryName);
	SW.Click("VPCorp_ScriptSearch_BN");
