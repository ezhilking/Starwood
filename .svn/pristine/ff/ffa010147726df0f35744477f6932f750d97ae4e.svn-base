...
public class MUSTRUN13_Reg_VP_PropertyContent_AttractiveServType {
	
	CRS SW =new CRS();
	String SucessfullyCreated;
	String AttractTypeSelected = SW.RandomString(5).toUpperCase();
...
//	String PolicyName="Automation Cancel Policy on"+ SW.GetTimeStamp("yyyy/MM/dd HH:mm:ss");
	...
	public void VPLogin(){
		...
		Reporter.Write("Login Created", "creation of login"," created", "pass");
	}
	@Test(priority=2)
	public void PropertyContent(){
		SW.NormalClick("VP_MenuSearch_EB");
		SW.EnterValue("VP_MenuSearch_EB","Attributes Service Type");
		SW.Click("VPPropContent_AttractionTypeSelected_BT");
		SW.SwitchToFrame("VP_MainFrame_FR");
	...
	public void CreateQS(){
		...
		SW.EnterValue("VPPropContent_AttractionType_ST", AttractTypeSelected);
	...
	public  void Db(){
		try{	
			...
			String query="select * from prop.srvc_type_lkup where srvc_type_name='"+ AttractTypeSelected +"'";
			Reporter.WriteLog(Level.INFO, query);
			if(SW.RecordExists(query)){
				List<String> output = SW.GetColumnValues(query, 1);{
					if (SW.CompareText(output.get(0), AttractTypeSelected)) {
