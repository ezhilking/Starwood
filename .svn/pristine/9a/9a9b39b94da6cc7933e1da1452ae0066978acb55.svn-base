...
public class MUSTRUN42_VPPropertyContentGolfType {
	CRS SW =new CRS();
	String SucessfullyCreated;
	String GolfType= SW.RandomString(5).toUpperCase();
	String GolfTypeID= SW.RandomString(2).toUpperCase();
	...
	public void PropertyContent(){
		...
		SW.EnterValue("VP_MenuSearch_EB","Golf Type");
		SW.Click("VPPropContent_ContentGolfType_BT");
		SW.SwitchToFrame("VP_MainFrame_FR");
	...
	//	SW.EnterValue("VPPropContent_CompassID_ET", GolfTypeID);
		SW.EnterValue("VPPropContent_AttractionType_ST",GolfType );
	...
	public  void Db(){
		try{	
			...
			String query="select golf_type_name from prop.golf_type_lkup where golf_type_name='"+ GolfType +"'";
			Reporter.WriteLog(Level.INFO, query);
			if(SW.RecordExists(query)){
				List<String> output = SW.GetColumnValues(query, 1);{
					if (SW.CompareText(output.get(0), GolfType)) {
