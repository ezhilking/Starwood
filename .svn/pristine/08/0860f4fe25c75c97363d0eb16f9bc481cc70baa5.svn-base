...
public class MUSTRUN44_VPPropertyMediaName {
	CRS SW =new CRS();
	String SucessfullyCreated;
	String MediaName= SW.RandomString(5).toUpperCase();
	String MediaID= SW.RandomString(2).toUpperCase();
	...
	public void PropertyContent(){
		...
		SW.EnterValue("VP_MenuSearch_EB","Media Name");
		SW.Click("VPPropContent_MediaName_BT");
		SW.SwitchToFrame("VP_MainFrame_FR");
	...
	//	SW.EnterValue("VPPropContent_CompassID_ET", MediaID);
		SW.EnterValue("VPPropContent_AttractionType_ST", MediaName);
	...
	public  void Db(){
		try{	
			...
			String query="select media_name_desc from prop.media_name_lkup where media_name_desc='"+ MediaName +"'";
			Reporter.WriteLog(Level.INFO, query);
			if(SW.RecordExists(query)){
				List<String> output = SW.GetColumnValues(query, 1);{
					if (SW.CompareText(output.get(0), MediaName)) {
