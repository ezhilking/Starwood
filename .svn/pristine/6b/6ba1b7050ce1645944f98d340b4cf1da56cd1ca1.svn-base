/* Purpose		: Verify appropriate error message is shown when successful reservation is reprocessed
 * TestCase Name: Verify appropriate error message is shown when successful reservation is reprocessed
 * Created By	: Muneeb
 * Modified By	: 
 ...
 */package testscripts.resCon;
 ...
 public class SMOKE07_ResconMpFlowReprocess {
	 CHANNELS SW = new CHANNELS();
	 String EventName = SW.RandomString(6);
	 String MailID;
	 String Xpath;
	 String password;
	 String ErrorMsg,crsConf;
	...
	//	 Environment.SetBrowserToUse("FF");
	 ...
	 public void Admin()
	 {
		 ...
		 SW.SwitchToWindow(2);
		 //		if(SW.ObjectExists("ResconMpFlow_FileContentBrowse_BT"))
		 //		{
		 //			Reporter.Write("ResconMpFlow_FileContentBrowse_BT", "ResconMpFlow_FileContentBrowse_BT is enabled", "ResconMpFlow_FileContentBrowse_BT is enabled", "PASS");
		 //		}
	 ...
	 public void MPFIdentity(){
		...
		SW.EnterValue("ResconLogin_Password_EB", SW.TestData("SGP_Password"));
		 ...
		 SW.Click("ResconMpFlow_ReProcessBtnReviewTransferPg_BT");
		 if(SW.IsEnabled("ResconMpFlow_ReProcessBtnReviewTransferPg_BT", "Enabled")){
			 Reporter.Write("ResconMpFlow_ReProcessBtnReviewTransferPg_BT", "Reprocess button should be enabled", "Reprocess button is enabled", "PASS");
			 SW.HandleAlert(true);
		 }else{
			 Reporter.Write("ResconMpFlow_ReProcessBtnReviewTransferPg_BT", "Reprocess button should be enabled", "Reprocess button is enabled", "FAIL");
		 }

		 SW.Click("ResconMpFlow_TransferSummaryBtn_BT");
		 ErrorMsg=SW.GetText("ResconMpFlow_ErrorMsgTransferInfoPg_ST");
