...
/** Purpose		: Validate SNA St.Regis Pre-Stay email in Spanish language with SPG offer and 3 social media icons
 * TestCase Name: Validate SNA St.Regis Pre-Stay email in Spanish language with SPG offer and 3 social media icons
 * Created By	: Sachin
 * Modified By	: 
 * Modified Date: 
 * Reviewed By	:	
 * Reviewed Date:
 * Comments: There is no availability for ST.Regis  in QA3 hence checking for Westin brand 1005 prop
 */
...
public class GC_REG48_ValidateSNAStRegisPreStaySpanishLangSPGOffer3SocialMedia {

	CRM SW = new CRM();	
	String ReservationNumber,CancelationNumber,OfferTitle,UserName,Password;
	...
	public void StartTest(){
		...
		if(Environment.getRunEnvironment().equalsIgnoreCase("QA3")){
			RequestXMLFile= "GC_REG48_SNAWestinReservationSpanishQA3_1005.xml";
			PropertyID="1005";
		}
	...
	@Test(priority=2, dependsOnMethods="CreateSPGOffer")
	...
	public void ValidateEditGCSetup(){
		...
		if(SW.ObjectExists("GCSetUp_FacebookImagePreStay_CB")&& SW.ObjectExists("GCSetUp_FlickerImagePreStay_CB")){
			
			SW.CheckBox("GCSetUp_FacebookImagePreStay_CB", "ON");
			...
			SW.CheckBox("GCSetUp_FlickerImagePreStay_CB","ON");
			String TwiterURL="https://flicker.com";
			SW.EnterValue("GCSetUp_FlickerURL_EB", TwiterURL);
			//SW.WriteToEmailTestData(TestCaseName, "ValiadtionString3", TwiterURL);
			
						
			SW.CheckBox("GCSetUp_InstagramImagePreStay_CB","ON");
	...
	@Test(priority=8, dependsOnMethods="CreateNewReservation")
