...
public class GC_REG43_ValidateSNAWestinCancellationEmailPortugueseSPGOffer4SocialMedia {
	...
	@Test(priority=1)
	public void CreateSPGCancellationOffer(){
		try{
			...
			if(SW.ObjectExists("GCCreateResConf_Rank_EB")){
				Reporter.Write("", "Rank button should present", "Rank button is present", "Pass");
				Environment.loger.log(Level.INFO,"Landing Page naviagation SuccessFull");
			}else{
				Reporter.Write("", "Rank button should present", "Rank button is not present", "Fail");
	...
	@Test(priority=2, dependsOnMethods="CreateSPGCancellationOffer")
	...
	public void ValidateEditGCSetup(){
		...
		if(!SW.ObjectExists("//input[@title='Edit Active']")){
			SW.Click("//span[@id='filterLink5']/a");
			SW.WaitTillElementToBeClickable("//input[@name='searchCriteriaValue']");
			SW.EnterValue("//input[@name='searchCriteriaValue']", PropID);
		...
		if(SW.ObjectExists("GCSetUp_TwiterImageResConf_CB")&& SW.ObjectExists("GCSetUp_FacebookImageResConf_CB")){
			...
			SW.CheckBox("GCSetUp_FacebookImageResCan_CB", "ON");
			...
			SW.CheckBox("GCSetUp_TwiterImageResCan_CB","ON");
			...
			SW.CheckBox("GCSetUp_FlickerImageResCan_CB","ON");
			...
			SW.CheckBox("GCSetUp_InstagramImageResCan_CB","ON");
