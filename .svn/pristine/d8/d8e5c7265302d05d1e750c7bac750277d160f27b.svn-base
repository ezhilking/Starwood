...
/** Purpose		: This is to validate the Event Posting in SPG Link 2.0 under SPG Pro User Role with CancelledMbr
 * TestCase Name: EventPostingCancelledMember
...
public class SPGLink_REG15_EventPostingCancelledMember {
	CRM SW = new CRM();
	String Mbrshp_num ; //Cancelled membership# Event should not be posted.
	...
	@Test(priority=0)
	public void DataSetupCancelledMember() {
		SW.EstablishConnection("QA3");
		List<String> MemberShip=SW.GetColumnValues("select * from freq_travel_mbrshp where mbrshp_sts='Y' and mbrshp_lvl='A' and mrktg_pgm='5' and src_cd='DIGA' and join_date > (sysdate-365)", "Mbrshp_num");
	...
	@Test(priority=1,dependsOnMethods="DataSetupCancelledMember")
	public void EventPostingCancelledMbr(){
		...
		SW.DropDown_SelectByText("SPGLinkEventPosting_MarketSegment_DD", "Corporate");
		SW.DropDown_SelectByText("SPGLinkEventPosting_EventType_DD", "Group Rooms With Function/Meeting Space");
		...
		if (SW.ObjectExists("SPGLinkEventPosting_Success_DT")) {
			...
			SW.FailCurrentTest("Cancelled Event cannot be created");
