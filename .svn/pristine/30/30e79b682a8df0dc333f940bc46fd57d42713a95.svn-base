...
/** Purpose		: This is to Enroll SPGPro Admin member in SPG Link 2.0 under SPG Pro User Role
 * TestCase Name: EnrolSPGProEnrollment_Admin
 * Created By	: Veeresh
 * Modified By	: 
 * Modified Date: 
 * Reviewed By	: 
...
public class SPGLink_REG06_EnrolSPGPro_Admin {
	...
	public void SPGProEnrollment() {
		...
		if (SW.ObjectExists("SPGLinkAdmin_RoleChangeAlert_DT")) {
			...
			Environment.loger.log(Level.INFO, "Admin mbr Enrolled Successfully!!"
					+ Enrollnum);

		} else {
			Environment.loger.log(Level.ERROR, "Enrollment fails");
			SW.FailCurrentTest("Validation fails for Admin enrollment");
