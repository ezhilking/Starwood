...
public class MUSTRUN21_DailyRatePlan {
	...
	@Test(priority=1)
	public void BarRatePlan()
	{
		SW.SwitchToFrame("VP_MainFrame_FR");
		//SW.GetText(Identifier)
		//SW.EnterValue("VPProp_PropRateDiffertials_FR", 1215);
		DailyRatePlan= SW.GetAttributeValue("VPProp_DailyRatePlan_DT", "value");
		Reporter.WriteLog(Level.INFO,DailyRatePlan);
	...
	public void RateListForm(){
		...
		if (SW.ObjectExists("VpRates_RateListForm_EB")) {
			SW.EnterValue("VpRates_RateListForm_EB","DailyRatePlan");		
		} else {
			SW.Wait(20);
			SW.EnterValue("VpRates_RateListForm_EB","DailyRatePlan");
		}
...
//			SW.CheckBox("VpRates_RatePlanId_CB", "ON");
//		}
			SW.VPClick("VPPropContent_HSCreate_BT");
	...
	public void RateSeasonPricing(){	
		...
		//	SW.VPClick("VpRates_SuperiorRoomClass_CB");//remove
			...
			SW.EnterValue("VPRates_ExtraChildRateR1_EB", 300);
	}
//	@Test(priority=4)
...
//		SW.SwitchToWindow(1);
//		System.out.println(SW.GetTitle());
//		SW.Click("VPPropContent_RatePLanInfoWIndow_BT");
//		SW.HandleAlert(true);
...
//		if (SW.ObjectExists("VPRates_CriticalErrMsg_BT")) {
