...
/** Purpose		: This is to validate the Event Posting in SPG Link 2.0 under SPG Pro User Role with two Member ( One Active and one Expired)
 * TestCase Name: EventPostingActiveExpired
...
public class SPGLink_REG12_EventPostingForTwoMbrs {
	CRM SW = new CRM();
	String Mbrshp_num ; //Active membership# 
	String Mbrshp_num2 ; // Second Membership Expired#
	...
	@Test(priority=0)
	public void DataSetupActiveExpiredMember() {
		...
		List<String> ExpiredMbr=SW.GetColumnValues("select * from freq_travel_mbrshp where mbrshp_sts='E' and mbrshp_lvl='A' and mrktg_pgm='5' and src_cd='DIGA' and join_date > (sysdate-365)", "Mbrshp_num");
			Mbrshp_num2=ExpiredMbr.get(0);
			
			if(Mbrshp_num != null && Mbrshp_num2 != null)
				Environment.loger.log(Level.INFO, "Member Exists " +Mbrshp_num +" and "+Mbrshp_num2);
			else
				SW.FailCurrentTest("No Member Exists, Check Query criteria");
			
		}
		@Test(priority=1,dependsOnMethods="DataSetupActiveExpiredMember")
		public void EventPostingActiveExpiredMbr(){
		...
		SW.EnterValue("SPGLinkEventPosting_AddAnother_EB", Mbrshp_num2);
