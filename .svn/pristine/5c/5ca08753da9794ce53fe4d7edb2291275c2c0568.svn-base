...
/* Purpose		: This script for Mask Credit Card on Profile Payment Screen-Proper Format -Mask after 30 Seconds-Negative
 * TestCase Name: Mask Credit Card on Profile Payment Screen-Proper Format -Mask after 30 Seconds-Negative
...
public class REG53_MaskCreditCard_ProperFormat_Negative {
	...
	public void checkCreditCardMask() {
		int CreditCardNumber = SW.RandomInteger(6);
		SW.WaitTillElementToBeClickable("NavigatorSearchPage_GuestNAme_DT");
		...
		SW.Wait(30);//waiting for 30 second ,CC don't mask
		String getCardInformation = SW.GetText("NavigatorSearchPage_CredeitCardNumber_EB"); //Get the credit card information after save
		if(SW.CompareText("CrCardLastFourDigit_comparision", Integer.toString(CreditCardNumber), getCardInformation)) //Comparing the card number
