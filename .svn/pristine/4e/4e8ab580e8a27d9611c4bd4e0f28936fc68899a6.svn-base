...
public class MUSTRUN36_CreateorModifyRP_VerifyErrorMessageDisplayedWhenDifferentialValueEnteredIsLowerThanGovernorValue {
	CRS SW = new CRS();
	...
	@Test(priority=0)
		...
		SW.EnterValue("VPRates_ChildRPSinglediff_EB", "5");
		SW.VPClick("VPRates_DynamicSeasonSave_BT");
		SW.CompareTextContained("VPRates_GovernorDiffErrorValidationForValueBelowGoverner", "Rate Plan differential value must be within Governor value range: 10 and 20 for SUN", SW.GetText("VPRates_ErrMsg_DT").trim());
		SW.EnterValue("VPRates_ChildRPSinglediff_EB", "25");
		SW.VPClick("VPRates_DynamicSeasonSave_BT");
		SW.CompareTextContained("VPRates_GovernorDiffErrorValidationForValueBelowGoverner", "Rate Plan differential value must be within Governor value range: 10 and 20 for SUN", SW.GetText("VPRates_ErrMsg_DT").trim());
		SW.EnterValue("VPRates_ChildRPSinglediff_EB", "20");
		SW.VPClick("VPRates_DynamicSeasonSave_BT");
