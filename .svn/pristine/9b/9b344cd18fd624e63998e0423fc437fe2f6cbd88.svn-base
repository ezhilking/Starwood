...
/* Purpose		: 
 * TestCase	 	:Validate the Corporate alerts in Galaxy in both modiy mode and check in mode for the guest having
 ...
 * Reviewed Date: 
 */
public class GalaxyPMS_REG05_Alerts {
	CRM SW = new CRM();
	...
	public void SGRVIPAndOptIn(){
		...
		//String ResponsefilePath="C:\\Users\\sachin.ganapathi\\Documents\\GC Automation\\SoapUI\\Booking Response_QA3.xml";
	...
	@Test(priority=3,dependsOnMethods="SGRCreateAlert")
	public void GalaxyALertVerification(){
		SW.NavigateTo(Environment.GALAXYURL);
		SK.GalaxyPMSLogin(ReservationNo);
		SW.Wait(3);
		if(SW.ObjectExists("")){//TODO Check for he alert icon
