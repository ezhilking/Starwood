...
public class MEETINGS_REG27_LinkDownloadWedding {
	...
	public void BookingCorporateWebsite() throws InterruptedException{
		...
		if(SW.ObjectExists("MeetingsCorporate_Link_LK")){
			...
			Reporter.Write("MeetingsWedding_Validate_ST_", "Google link is present", "Google link is present", "PASS");
		}else{
			Environment.loger.log(Level.ERROR, "Fail: Google link is not present");	
			Reporter.Write("MeetingsWeddingLink_Validate_ST_", "Google link is not present", "Google link is not present", "FAIL");
		}
		...
		if(SW.CompareText("ValidationWeddingPDF_ST", "HISTORICAL PICKUP", pdfText))
		{
