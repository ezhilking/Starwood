...
/* Purpose		: Valdiate if user is able to create another file when a GHM file is created with the same resconf number
 * TestCase Name: REG14_Validate_GHMFile_for_SameConfNum.java
 ...
 * Reviewed Date:
 */
public class REG14_Validate_GHMFile_for_SameConfNum {
	CRM SW = new CRM();
	String CSFFileNo;
	String sPropId = "1965";
	...
	public void ValidateMatchingGCD() {
		...
		SW.EnterValue("CSFResConf_ToDate_EB", SW.DateAddDays(
				SW.GetTimeStamp("dd MMM yyyy"), "dd MMM yyyy", -14,
				Calendar.DATE));
		...
		CSFFileNo = CSFText.substring(CSFText.indexOf("(") + 1,
				CSFText.indexOf(")"));
		Environment.loger.log(Level.INFO, "Your CSF File Number is "
				+ CSFFileNo);
		...
		SW.SelectRadioButton("CSFHome_GuestYes_RB");
		SW.DropDown_SelectByText("CSFHome_TypeCreate_DD", "Customer Service");
		SW.NormalClick("CSFHome_CreateNewCSF_BN");
		SW.Wait(2);
		if (!SW.ObjectExists("CSFSummary_IntialContact_DD")) {
			Environment.loger.log(Level.INFO, "CSV File Already Exsist as Expected message has Displayed");
		} else {
			Environment.loger.log(Level.ERROR, "Allow to create new CSF file for Same Resconf Number");
			SW.FailCurrentTest("Validation fails in Create new file on same");
		}
