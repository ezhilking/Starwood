...
/** Purpose		: This is to Enroll SPGPro MP Admin member in SPG Link 2.0 under SPG Pro User Role
 * TestCase Name: EnrolSPGProEnrollment_MPAdmin
 * Created By	: Veeresh
 * Modified By	: 
 * Modified Date: 
 * Reviewed By	: 
...
public class SPGLink_REG08_EnrolSPGPro_MPAdmin {
	...
	public void SPGProEnrollment() {
		...
		SW.SelectRadioButton("SPGLinkEnroll_TravelProfessionalNo_RB");
		SW.SelectRadioButton("SPGLinkEnroll_Admin_RB");
		SW.EnterValue("SPGLinkEnroll_SETNo_EB", SW.TestData("QA3_SETNo"));
		SW.DropDown_SelectByValue("SPGLinkEnroll_Admin_CustomerBookedPerYear_DD", "02");
		SW.DropDown_SelectByValue("SPGLinkEnroll_Admin_RoomNightsBookedPerYear_DD", "03");
		SW.SelectRadioButton("SPGLinkEnroll_Admin_TandC_RB");
		...
		if (SW.ObjectExists("SPGLinkAdmin_RoleChangeAlert_DT")) {
			...
			Environment.loger.log(Level.INFO, "combination of Meeting Planner & Travel Professional mbr Enrolled Successfully!!"
					+ Enrollnum);

		} else {
			Environment.loger.log(Level.ERROR, "Enrollment fails");
			SW.FailCurrentTest("Validation fails for combination of Meeting Planner & Travel Professional enrollment");
