...
/** Purpose		: This is to validate the mandatory field "starpoints" under Misc starpoints role in goodwill points request page 
 * TestCase Name: GoodwillNullPointRequest
...
public class SPGLink_REG34_GoodwillNullPointRequest {
	 CRM SW = new CRM();
		String Mbrshp_num;
		...
		@Test(priority=1)
		public void getDataGoodwillNullPointRequest() {
			SW.EstablishConnection("QA3");
			String query = "select * from freq_travel_mbrshp where mbrshp_lvl='A' and mbrshp_sts='A' and join_Date > (sysdate-6) and src_cd='DIGA'";
			Mbrshp_num=SW.GetColumnValues(query, "MBRSHP_NUM").get(0);
			System.out.println(Mbrshp_num);
		}
		
		@Test(priority=2,dependsOnMethods="getDataGoodwillNullPointRequest")
		public void GoodwillNullPointRequest() {
			...
			SW.EnterValue("SPGLink_OtherStarPoints_StarPoints_EB","0");
			SW.Click("SPGLink_OtherStarPoints_ConfNum_EB");
			SW.ClickAndProceed("SPGLink_OtherStarPoints_Submit_BT");
			SW.Wait(5);
			SW.HandleAlert(true);
			
			/* system should throw an exception notifying Starpoints is a mandatory field, it cannot be null */
