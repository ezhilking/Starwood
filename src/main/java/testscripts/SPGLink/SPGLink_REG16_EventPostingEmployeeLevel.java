...
/** Purpose		: Negative_Testing : This is to validate the Event Posting in SPG Link 2.0 under SPG Pro User Role with Employee Member
 * TestCase Name: EventPostingEmployeeMember
...
public class SPGLink_REG16_EventPostingEmployeeLevel {
	CRM SW = new CRM();
	String Mbrshp_num ; //EmployeeLevel membership# Event should not be posted.
	...
	@Test(priority=0)
	public void DataSetupEmpMember() {
		SW.EstablishConnection("QA3");
		List<String> MemberShip=SW.GetColumnValues("select * from freq_travel_mbrshp where mbrshp_sts='A' and mbrshp_lvl='E' and mrktg_pgm='5' and src_cd='DIGA' and join_date > (sysdate-65)", "Mbrshp_num");
	...
	@Test(priority=1,dependsOnMethods="DataSetupEmpMember")
	public void EventPostingEmpLevel(){
		...
		if (SW.ObjectExists("SPGLinkEventPosting_Success_DT")) {
			...
			SW.FailCurrentTest("Employee Level cannot post Event");
