...
/** Purpose		:	Notification Triggered when  Tier Renewal membership notification 
 * TestCase Name:	REG16_TierRenewal.java
 ...
 * Reviewed Date: 
 */
public class REG16_TierRenewal {
	CRM SW = new CRM();
	String sUsername, sPassword, sGripUsrName, sGripPassword, sAppname;
	String spgMem;// use A level SPG Member
	List<String> sExp_Date;
	String sStartDate = "2016-05-19";// Change the arrival date
	...
	{
		//int SPGNum=Integer.parseInt(spgMem);
		String date=SW.DateAddDays(SW.GetTimeStamp("dd-MMM-yy"), "dd-MMM-yy", 15, Calendar.DATE);
		SW.EstablishConnection("qa4");
		String UpdateQuery="UPDATE freq_travel_mbrshp SET exp_date='"+date+"' where mbrshp_num = '42008413112'"; //pass the SPGMEMBER which is used in Excel sheet.
		SW.UpdateQuery(UpdateQuery);
		String getQuery="SELECT * FROM freq_travel_mbrshp WHERE mbrshp_num='42008413112'"; //pass the SPGMEMBER which is used in Excel sheet.
		sExp_Date=SW.GetColumnValues(getQuery, "exp_date");
	}
	@Test(priority=2)
	...
	@Test(priority=3, dependsOnMethods="PointPostedStay")
	public void dbValidtion()
	{
		try{	
			...
			SW.EstablishConnection("qa4");
			String CheckQuery="select * from odsft.freq_travel_notf_hist where mbrshp_num in ("+spgMem+") and message_type_id in ('894','892')";
			...
			// Checking the Expire date is updated
			String getQuery="select EXP_DATE from freq_travel_mbrshp where mbrshp_num ='42008413112'"; //pass the SPGMEMBER which is used in Excel sheet.
			List<String> ExpDate=SW.GetColumnValues(getQuery, 1);
			if(!sExp_Date.equals(ExpDate))
				Environment.loger.log(Level.INFO, "Expire Date has Updated");
			else
			{
				Environment.loger.log(Level.ERROR,"Date not Get Update");
				SW.FailCurrentTest("Expire date not updated");
			}
			CheckQuery="select * from freq_travel_call_comm where call_comm_Cd='WCT' and mbrshp_num in ("+spgMem+")and call_comm_text in ('STARPOINTS POSTED FOR A STAY.','ACCOUNT UPGRADED OR RENEWED.')";
			if(SW.RecordExists(CheckQuery))
