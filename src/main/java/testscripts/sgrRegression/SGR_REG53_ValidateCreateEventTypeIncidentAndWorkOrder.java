...
public class SGR_REG53_ValidateCreateEventTypeIncidentAndWorkOrder {
	CRM SW = new CRM();
	...
	@Test(priority=2, dependsOnMethods="CreateIncidentEventForGuest")
	public void CreateWorkOrderEventForGuest(){
		SW.Click("SGRNavigation_Home_LK");
		SW.Wait(15);
		SW.SwitchToFrame("SGRHomepage_InHouse_FR");
		SW.SwitchToFrame("SGRHomepage_InHouseSVOQI_FR");
		...
		SW.Click("SGRCreateEvent_Save_BN");
		EventID=SW.GetEventNumbeID();
		Environment.loger.log(Level.INFO, "Created Work Order Event ID - "+EventID);
	}
