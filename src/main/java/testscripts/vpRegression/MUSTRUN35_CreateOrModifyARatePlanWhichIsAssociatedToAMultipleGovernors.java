 ...
 * TestCase Name:  Reg_Corp User_Create or Modify RP_Verify user is able to Create or Modify a rate plan which is associated to a multiple governors where all meet the required specifications.
...
public class MUSTRUN35_CreateOrModifyARatePlanWhichIsAssociatedToAMultipleGovernors {
	CRS SW = new CRS();
	...
	@Test(priority=0)
		...
		//***************************************** Creating Second Governor ***************************************************************
		
		SW.WaitForObject("VP_CorpRPGoverners_CreateGov_BT");
		SW.VPClick("VP_CorpRPGoverners_CreateGov_BT");
		SW.Click("VP_CorpRPGoverners_Division_CB");			
		SW.EnterValue("VP_CorpRPGoverners_RPIdCorpLvl_EB", RatePlanID);
		SW.EnterValue("VP_CorpRPGoverners_MinPercentageAmount_EB", "15");
		SW.EnterValue("VP_CorpRPGoverners_MaxPercentageAmount_EB", "30"+Keys.TAB);
		SW.Click("VP_CorpRPGoverners_Submit_BN");
		SW.CompareTextContained("Governer Creation Sucess Msg Validation", "Governor Saved Successfully.",SW.GetText("VP_CorpRPGoverners_SuccessMsg_DT"));
