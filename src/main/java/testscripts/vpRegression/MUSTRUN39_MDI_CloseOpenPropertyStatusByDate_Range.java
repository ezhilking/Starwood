/* Purpose		:  Valhalla Portal Must Run
 * TestCase Name:  MUSTRUN_MDI_EditPropertyStatusByDate_Range
...
public class MUSTRUN39_MDI_CloseOpenPropertyStatusByDate_Range {
	...
	{
		SW.DropDown_SelectByIndex("VPInventory_HouseLevelStatus_ED", 4);
		...
		String query="select inv_sts_cd from inventory.prdct_inv where PRDCT_ID = '30926' and use_start_dtme >= SYSDATE and use_end_dtme <= SYSDATE + 3";
		List<String> inv_status = SW.GetColumnValues(query, "inv_sts_cd");
		int status = 0;
		...
		if(status == 0)
			Reporter.Write("DB Validation", "Inventory Status should be Closed sucessfully in DB", "Inventory Status has been Closed sucessfully in DB", "PASS");
		else
			Reporter.Write("DB Validation", "Inventory Status should be Closed sucessfully in DB", "Inventory Status not Closed sucessfully in DB", "FAIL");
	...
	@Test(priority=2, dependsOnMethods={ "viewModifyInventory" , "closeInventory" })
