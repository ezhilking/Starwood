...
public class MUSTRUN45_VPPropertyMeetingPrimarySetup {
	CRS SW =new CRS();
	String SucessfullyCreated;
	String MeetingPrimaryName= SW.RandomString(5).toUpperCase();
	String MeetingPrimaryNameID= SW.RandomString(2).toUpperCase();
	...
	public void PropertyContent(){
		...
		SW.EnterValue("VP_MenuSearch_EB","Meeting Primary Set-up Type");
		SW.Click("VPPropContent_MeetingPrimarySetup_BT");
		SW.SwitchToFrame("VP_MainFrame_FR");
	...
	//	SW.EnterValue("VPPropContent_CompassID_ET", MeetingPrimaryNameID);
		SW.EnterValue("VPPropContent_AttractionType_ST",MeetingPrimaryName );
	...
	public  void Db(){
		try{	
			...
			String query="select mtg_space_type_name from prop.mtg_space_type_lkup where mtg_space_type_name='"+ MeetingPrimaryName +"'";
			Reporter.WriteLog(Level.INFO, query);
			if(SW.RecordExists(query)){
				List<String> output = SW.GetColumnValues(query, 1);{
					if (SW.CompareText(output.get(0), MeetingPrimaryName)) {
