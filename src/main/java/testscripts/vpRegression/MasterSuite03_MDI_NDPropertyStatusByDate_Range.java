...
public class MasterSuite03_MDI_NDPropertyStatusByDate_Range {
	...
	public void noDepartureInventory()
	{
		SW.DropDown_SelectByIndex("VPInventory_HouseLevelStatus_ED", 3);
		...
		if(status == 0)
			Reporter.Write("DB Validation", "Inventory Status should be ND sucessfully in DB", "Inventory Status has been ND sucessfully in DB", "PASS");
		else
			Reporter.Write("DB Validation", "Inventory Status should be ND sucessfully in DB", "Inventory Status not ND sucessfully in DB", "FAIL");
	...
	@Test(priority=2, dependsOnMethods={ "viewModifyInventory" , "noDepartureInventory" })
