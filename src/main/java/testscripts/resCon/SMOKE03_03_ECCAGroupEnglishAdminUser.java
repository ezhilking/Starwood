...
/* Purpose		: Validating ECCA GroupEnglish_admin user
 * TestCase Name: TC5_Verify viewing the eCCA form contents_Group Type_English Language
...
public class SMOKE03_03_ECCAGroupEnglishAdminUser {
	...
	public void CreateECCAGroupEnglishInvite(){
		...
		SW.DropDown_SelectByValue("ResconECCA_GroupSelectProperty_DD", SW.TestData("SGP_PID"));
		...
		SW.Click("ResconECCA_EventChargesBanquet/CateringEventCharges_CB");
		SW.Click("ResconECCA_EventChargesRoom & Tax Charges_CK");
		...
		if(SW.IsEnabled("ResconECCA_GroupSubmissionSubmission_BT","Enabled")){
			Reporter.Write("All the entered in the ECCA form is valid and click on sumbit button", "Sumbit button should be disabled", "sumbit button is disabled", "Pass");
		}else{	
			Reporter.Write("All the entered in the ECCA form is valid and click on sumbit button", "Sumbit button should be disabled", "sumbit button is not disabled", "Fail");
		}
