...
public class GC_REG67_ValidateSheratonAHBBSBSPGuestEmailEngLangNDS {
	CRM SW = new CRM();	
	...
	String OfferHead,OfferSubHead;String TestCaseName= getClass().getName();
	@BeforeClass
	...
	@Test(priority=1)
	public void CreateAHBBSBSPOfferEnglishLang(){
		...
		EmailSub ="Save up to 25% UAT"+SW.RandomString(15);
		SW.EnterValue("GCAHBBMPMOffer_EmailSetup_Subject_EB", EmailSub);
		EmailPreHeader="A long p-header check."+SW.RandomString(10);
		...
		OfferHead ="Save now on summer Test.<br> Offer Breakfast and Spa Package SAMPLE"+SW.RandomString(10);
		...
		SW.EnterValue("GCAHBBMPMOffer_MessageSetupOfferDescription_EB","QA is here, the weather is warming up and it is the perfect time to TEST! Don't miss great things to do like UAT in Chicago, the 17th Annual QA Fair in St. Louis, a UAT Tour in Boston and The Minnesota State Fair. Take advantage of great UAT at participating hotels below in Chicago, St. Louis, Boston and Minneapolis.");
		SW.SwitchToFrame("");
		...
		SW.EnterValue("GCAHBBMPMOffer_SecondaryPageUrl_EB", "http://www.starwoodhotels.com");
		SW.Click("GCAHBBMPMOffer_Continue_BN");
		SW.WaitTillElementToBeClickable("GCAHBBMPMOffer_Continue_BN");
		SW.Wait(5);
	...
	@Test(priority=2,dependsOnMethods="CreateAHBBSBSPOfferEnglishLang")
