...
/** Purpose		: This is to Validate Sheraton Instay departure document in English language
 * TestCase Name:  Validate Sheraton Instay departure document in English language
...
public class GC_REG59_ValidateSheratonInStayDepartureDocEngLang {
	CRM SW = new CRM();	
	...
	public void StartTest(){
		...
		if(Environment.getRunEnvironment().equalsIgnoreCase("QA3")){
			RequestXMLFile= "GC_REG59_SheratonEnglish_1047_QA3.xml";
			sPropertyID="1047";
		}
		if(Environment.getRunEnvironment().equalsIgnoreCase("QA2")){
			RequestXMLFile= "GC_REG59_QA2_Sheraton_376.xml";
			sPropertyID="376";
		}
	}
	@Test(priority=1)
	public void GCCreateInStayDepatureoffer(){
		try{
			...
			SW.Click("GCCreateInstayDepat_LK");
	...
	@Test(priority=2,dependsOnMethods={"GCCreateInStayDepatureoffer"})
	...
	public void DataBaseUpdate(){
		try{	
			...
			String UpdateQuery="update reservation_header set departure_dt =sysdate+1 where reservation_confirmation_num in ("+ReservationNumber+")";
	...
	@Test(priority=6, dependsOnMethods="RunInStayTask")
	public void ValidateInStayDoc (){
		try{
			...
			if(SW.ObjectExists("GCCreateResConf_InstayDocDepTable_WT"))
			{
				List<String> sCnfNum=SW.GetAllText("GCCreateResConf_InstayDocDepResNum_WT");
				...
				if(sCnfNum.contains(ReservationNumber+" "))
				{
					...
					Reporter.Write("InStay Document Delivery", "InStay Document Delivery should be SuccessFull  ", "InStay Document Delivery SuccessFull  ", "PASS");
					if(SW.ObjectExists("GCNavigation_SignOut_LK")){
						SW.Click("GCNavigation_SignOut_LK");
					}
				}
