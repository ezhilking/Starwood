...
/** Purpose		: Validate Element AHBB SBMP email in Spanish language
 * TestCase Name: Validate Element AHBB SBMP email in Spanish language
...
public class GC_REG60_ValidateElementAHBBSBMPEmailSpanishLang {
	CRM SW = new CRM();	
	...
	String SectionHeader;//For first Prop
	...
	@Test(priority=1)
	public void CreateAHBBSBMPOfferSpanLang(){
		...
		SW.DropDown_SelectByText("GCCreateResConf_Language_DD","Spanish (Spain)");
		...
		EmailSub ="EmailSub"+SW.RandomString(10);
	...
	@Test(priority=2, dependsOnMethods="CreateAHBBSBMPOfferSpanLang")
	public void databaseUpdateforAHBB(){
		...
		try{	
			...
			if(SW.CompareText(BrandID.get(0),BrandID.get(1))){// to validate same brand for both the prop for SBMP
				...
				String UpdateQuery="update CRMSTG.GC_PROP_STAY set LAST_STAY_DT='25-Feb-12' where PROP_MASTER_ID in ("+Prop1+","+Prop2+") and CURRENT_GST_MASTER_PROF_ID in (SELECT GG.CURRENT_GST_MASTER_PROF_ID FROM CRMSTG.GC_PROP_STAY GPS,CRMSTG.GC_BRAND_STAY GBS,CRMSTG.GC_GUEST GG WHERE GG.CURRENT_GST_MASTER_PROF_ID = GPS.CURRENT_GST_MASTER_PROF_ID AND GG.CURRENT_GST_MASTER_PROF_ID = GBS.CURRENT_GST_MASTER_PROF_ID AND GG.EMAIL_ADDR_TEXT IS NOT NULL AND GG.EMAIL_ADDR_TEXT NOT LIKE '%AOL.COM' AND GPS.LAST_STAY_DT > =TRUNC( sysdate - 555) AND GPS.PROP_MASTER_ID IN  ("+Prop1+","+Prop2+") AND GBS.BRAND_ID IN ("+BrandID.get(0)+"))";
				SW.UpdateQuery(UpdateQuery);
				String UpdateQuery2="update CRMSTG.GC_BRAND_STAY set LAST_STAY_DT='25-Feb-12' where BRAND_ID in ("+BrandID.get(0)+") and CURRENT_GST_MASTER_PROF_ID in (SELECT GG.CURRENT_GST_MASTER_PROF_ID FROM CRMSTG.GC_PROP_STAY GPS,CRMSTG.GC_BRAND_STAY GBS,CRMSTG.GC_GUEST GG WHERE GG.CURRENT_GST_MASTER_PROF_ID = GPS.CURRENT_GST_MASTER_PROF_ID AND GG.CURRENT_GST_MASTER_PROF_ID = GBS.CURRENT_GST_MASTER_PROF_ID AND GG.EMAIL_ADDR_TEXT IS NOT NULL AND GG.EMAIL_ADDR_TEXT NOT LIKE '%AOL.COM' AND GPS.LAST_STAY_DT > =TRUNC( sysdate - 555) AND GPS.PROP_MASTER_ID IN  ("+Prop1+","+Prop2+") AND GBS.BRAND_ID IN ("+BrandID.get(0)+"))";
				SW.UpdateQuery(UpdateQuery2);

				// check for the existence of the GMP ID in the tables 
				String SelectQuery="select count(*) from CRMSTG.GC_BRAND_STAY  where CURRENT_GST_MASTER_PROF_ID= "+GMPID+" and BRAND_ID = "+BrandID.get(0)+"";
				boolean result=SW.RecordExists(SelectQuery);
				if(result){
					String UpdateQuery3="update CRMSTG.GC_BRAND_STAY set LAST_STAY_DT='24-Jun-16' where CURRENT_GST_MASTER_PROF_ID in ("+GMPID+") and BRAND_ID="+BrandID.get(0)+"";
					...
					String InsertQuery="Insert into CRMSTG.GC_BRAND_STAY(CURRENT_GST_MASTER_PROF_ID,BRAND_ID,LAST_STAY_DT,TOTAL_STAYS) values ("+GMPID+","+BrandID.get(0)+",'05-MAY-2016',5)";
				...
				String UpdateLanguageCode="update CRMSTG.GC_GUEST set LANGUAGE_CD='ESP' where CURRENT_GST_MASTER_PROF_ID in ("+GMPID+")";
				...
				String ValidateEligibleGuest="SELECT GG.CURRENT_GST_MASTER_PROF_ID,GG.EMAIL_ADDR_TEXT,GG.COUNTRY_TWO_CHAR_CD,GG.GST_FIRST_NAME,GG.GST_LAST_NAME FROM CRMSTG.GC_PROP_STAY GPS,CRMSTG.GC_BRAND_STAY GBS,CRMSTG.GC_GUEST GG WHERE GG.CURRENT_GST_MASTER_PROF_ID = GPS.CURRENT_GST_MASTER_PROF_ID AND GG.CURRENT_GST_MASTER_PROF_ID = GBS.CURRENT_GST_MASTER_PROF_ID AND GG.EMAIL_ADDR_TEXT IS NOT NULL AND GG.EMAIL_ADDR_TEXT NOT LIKE '%AOL.COM' AND GPS.LAST_STAY_DT > =TRUNC( sysdate - 555) AND GPS.PROP_MASTER_ID IN ("+Prop1+") AND GBS.BRAND_ID IN ("+BrandID.get(0)+")";
	...
	public void RunAHBBTask(){
		try{
			...
			Environment.loger.log(Level.ERROR, "Failed During AHBBTaskRun",e);
			SW.WriteToEmailTestData(TestCaseName, "ExecutionFlag", "N");
			SW.FailCurrentTest("AHBB Task Run failed");
		}
	}
