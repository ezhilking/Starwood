...
/* Purpose		: This script for Mask Credit Card on Profile Payment Screen- Mask After Leaving CC Number Field
 * TestCase Name: Mask Credit Card on Profile Payment Screen- Mask After Leaving CC Number Field
 ...
 * Reviewed Date:
 */
public class REG50_MaskCreditCard_AfterLeavingCCNumberField {
	...
	public void editPaymentInfo() {
		...
		if(SW.CompareText("CrCardLastFourDigit_comparoision", Expected_last4DigitCardNumber, getCardInformation)) //Comparing the card number
			Environment.loger.log(Level.INFO,"The Credit card number has masked as expected");
		else {
			Environment.loger.log(Level.INFO,"The Credit card Number has not masked");
			SW.FailCurrentTest("Validation Fails in checking CreditCard Mask");
		}
	}
