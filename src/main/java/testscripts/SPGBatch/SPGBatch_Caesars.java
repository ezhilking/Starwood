...
public class SPGBatch_Caesars {
	...
	@Test(priority=0)
	public void SPGBatch_Delta_DataSetup() {
		...
		String Query1="select Mbrshp_num from freq_travel_mbrshp where mbrshp_lvl='A' and mbrshp_sts='A' and join_date > (sysdate-30) and src_cd='WEBA'";
		List<String> MemberShip=SW.GetColumnValues(Query1, "Mbrshp_num");
		Mbrshp_num1=MemberShip.get(2);
		/*---------------------------------------------------------------------------------------------------------------------------*/
		
		/*Step 2: Query2 --> Check whether member has any external linked, if yes, delete it and link it with CAESARS */
		...
		String Query3="Insert into freq_travel_extl_mbrshp (MBRSHP_NUM,MRKTG_PGM,EXTL_MRKTG_PGM_ID,TRANSFER_TYPE,EXTL_MBRSHP_NUM,EXTL_MBRSHP_LVL,CREATE_USER_ID,CREATE_DATE,UPDATE_USER_ID,UPDATE_DATE,ACTV_DATE,DEACTV_DATE,EXTL_MBRSHP_ID) values"+" ('"+Mbrshp_num1+"',5,'CAESARS','N/A','"+Extl_id1+"','3','VAISKRI',to_timestamp('28-DEC-16','DD-MON-RR HH.MI.SSXFF AM'),null,null,"+Actv_Date+",null,"+Extl_id1+")";
		if(count>=1){
		...
		/*Step 4: Query 7 --> prc_ft_batch_parm date set up*/
		List<String> seq_num=SW.GetColumnValues("select seq_num from prc_ft_batch_parm where parm_name='QUAL_STAY_LAST_TMSTMP' and job_name='CSQUALST'", "seq_num");
		String seq_num1=seq_num.get(0);
		String Query7="update prc_ft_batch_parm set parm_value='"+Late_Qual_tmstmp+"' where seq_num='"+seq_num1+"' and job_name='CSQUALST'";
		SW.UpdateQuery(Query7);
		List<String> actv_seq_num=SW.GetColumnValues("select seq_num from prc_ft_batch_parm where parm_name='ACTV_START_DATE' and job_name='CSQUALST'", "seq_num");
		String actv_seq_num1=actv_seq_num.get(0);
		String Query8="update prc_ft_batch_parm set parm_value='"+SW.DateAddDays(SW.GetTimeStamp("dd-MMM-yy"), "dd-MMM-yy", -365,	 Calendar.DATE).toUpperCase()+"' where seq_num='"+actv_seq_num1+"' and job_name='CSQUALST'";
