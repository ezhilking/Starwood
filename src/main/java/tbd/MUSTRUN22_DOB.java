...
public class MUSTRUN22_DOB {
	...
	public void TransientRatePlan(){
		...
		SW.DoubleClick("VPRates_RPHeaderSave_BT");
		SW.SwitchToFrame("");
		SW.WaitForObject("VP_MainFrame_FR");
		SW.SwitchToFrame("VP_MainFrame_FR");
		SW.CheckBox("VPRates_Season_DD","ON");
		SW.ClickAndProceed("VPRates_SeasonAdd_DD");//TODO		
		SW.DoubleClick("VPRates_RPAssociationSave_BT");	
		SW.Wait(2);
		...
		SW.DoubleClick("VPRates_DynamicSeasonSave_BT");
		SW.DoubleClick("VPRates_DynamicSeasonSave_BT");
		if(SW.HandleAlert(true)){
			Environment.loger.log(Level.INFO, "Max los alert validation Passed");
		}else {
		...
		SW.DoubleClick("VPRates_SellSequanceSave_BT");
		SW.DoubleClick("VPRates_SellSequanceSave_BT");
		SW.Click("VPRates_RPAssociateChannelsSave_BT");
		Reporter.Write("Successfull info", "VPRates_RPinfoMsg_DT", "VPRates_RPinfoMsg_DT", "succesfull");
	}
	@AfterClass
	public void EndTest(){
		Reporter.StopTest();		
